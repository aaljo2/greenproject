...
const create = async (req, res) => {
  console.log("여기는 서버 컨틀로러 생성 2) ");
  const user = new SocialUser(req.body)
  try {
    await user.save()
...
const userByID = async (req, res, next, id) => {
  try {
    let user = await SocialUser.findById(id).populate('following', '_id name')
...
const list = async (req, res) => {
  try {
    let users = await SocialUser.find().select('name email updated created')
    res.json(users)
  } catch (err) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(err)
